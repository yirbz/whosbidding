...
export function StartupLogo({ handle, websiteUrl }: StartupLogoProps) {
  ...
  // Handle @handle X.com entries (renders official X profile avatar or X logo vector)
  if (isTwitter) {
  ...
  // Derive target domain for website startup entries
  const targetDomain = (() => {
    if (websiteUrl) {
  ...
  // Generate deterministic brand background color for initial letters fallback
